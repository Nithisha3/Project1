...
public class Claim implements IClaim {
	...
	public ClaimAdjuster addClaim(ClaimAdjuster cl) throws SQLException {
		...
		ClaimAdjuster ca = getClaim(cl.getClaimId());
		if (ca.getClaimId() == null) {
			...
			throw new SQLException("Claim Already Exists");
		}
		return cl;
	}
	...
	public ClaimAdjuster getClaim(String claimId) throws SQLException {
		...
		while (rsUser.next()) {
			...
			ca.setClaimNumber(rsUser.getInt("claimNumber"));
		}
		return ca;
	}
	...
	public void saveQuestionAndAnswer(String claimId, String answerNumbers) throws SQLException {
		psmt = con.prepareStatement("insert into report values(?,?)");
		psmt.setString(1, claimId);
		psmt.setString(2, answerNumbers);
		psmt.executeUpdate();
	}

	@Override
	public String getQuestionAndAnswer(String claimId) throws SQLException {
		psmt = con.prepareStatement("select answer from report where claimId = ?");
		psmt.setString(1, claimId);
		rsUser = psmt.executeQuery();
		rsUser.next();
		return rsUser.getString("answer");
	}
}
